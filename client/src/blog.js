  ...
  render() {
    return (
      <div className="cont">
        ...
        {!this.state.clicked ? (
          <button className="button-pick" onClick={this.PostClick}>
            {" "}
            Post new message{" "}
          </button>
        ) : (
          <div className="create-post">
            <h3>
              {this.state.name}, what would you like to say to the rest of your
              teammates?
            </h3>
            <input type="text" onChange={(e) => this.UpdateText(e)} />
            <button onClick={this.CreateBlog}>
              {this.state.text.length === 0 ? "Go back" : "Submit"}
            </button>
          </div>
        )}
        {this.state.blogList
